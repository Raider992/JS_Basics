...
const settings = {
    ...
    maxObstacleSize: 3,
    obstacleRefreshRate: 30             //обновлять положение препятствий раз в n ходов
};
...
const config = {
    ...
    getNumberOfObstacles() {
        return this.settings.numberOfObstacles;
    },

    getObstacleRefreshRate() {
        return this.settings.obstacleRefreshRate;
    },
...
const scoreField = {
    ...
    render(snakePointsArray, winScore) {
        this.currentScore = document.getElementById('currentScore');
        this.scoreToWin = document.getElementById('scoreToWin')

        this.currentScore.innerText = snakePointsArray.length - 1;
        this.scoreToWin.innerText = winScore;
    }
...
const snake = {
    ...
    getNextStepHeadPoint() {
        ...
        switch (this.direction) {
            case 'up':
                if (firstPoint.y - 1 < 0) {
                    return {x: firstPoint.x, y: config.getRowsCount()};
                } else return {x: firstPoint.x, y: firstPoint.y - 1};
            case 'right':
                if (firstPoint.x + 1 > config.getColsCount()) {
                    return {x: 0, y: firstPoint.y};
                } else return {x: firstPoint.x + 1, y: firstPoint.y};
            case 'down':
                if (firstPoint.y + 1 > config.getRowsCount()) {
                    return {x: firstPoint.x, y: 0};
                } else return {x: firstPoint.x, y: firstPoint.y + 1};
            case 'left':
                if (firstPoint.x - 1 < 0) {
                    return {x: config.getColsCount(), y: firstPoint.y};
                } else return {x: firstPoint.x - 1, y: firstPoint.y};
        }
    },
...
const obstacle = {
    ...
    generateObstacles() {
        if (this.obstaclePointsArray) this.obstaclePointsArray.splice(0);    //Для пущего хардкора можно закомментировать, тогда число препятствий будет увеличиваться =)
        for (let i = 0; i < config.getNumberOfObstacles(); i++) {
            let obstacle = Object.assign({}, game.getRandomFreeCoordinates());
            this.obstaclePointsArray.push(obstacle);
        }
    },

    getObstaclesArray() {
        return this.obstaclePointsArray;
    },
...
const game = {
    ...
    render() {
        this.map.render(this.snake.getBody(), this.food.getCoordinates(), this.obstacle.obstaclePointsArray);
        this.scoreField.render(this.snake.getBody(), this.config.getWinFoodCount());
    },
    ...
    tickHandler() {
        ...
        if(this.snake.stepCounter > this.config.getObstacleRefreshRate()) {
            this.obstacle.generateObstacles();
            this.snake.zeroStepCounter();
        }
        ...
        if (this.food.isOnPoint(this.snake.getNextStepHeadPoint())) {
            this.snake.growUp();
            this.food.setCoordinates(this.getRandomFreeCoordinates());
            //this.obstacle.generateObstacles();                        //один из вариантов решения, который видел в змейках - генерировать препятствия в момент подбора еды
    ...
    canMakeStep() {
        ...
        const obstaclesArray = this.obstacle.getObstaclesArray();
        ...
        return !this.snake.isOnPoint(nextHeadPoint)
            ...
            // && nextHeadPoint.x >= 0
            // && nextHeadPoint.y >= 0
            && !obstaclesArray.some(obstaclePoint => obstaclePoint.x === nextHeadPoint.x && obstaclePoint.y === nextHeadPoint.y);
    },
    ...
    getRandomFreeCoordinates() {
        const exclude = [this.food.getCoordinates(), ...this.snake.getBody(), ...this.obstacle.getObstaclesArray()];
